...
function zeropool(contractAddress, privateKey) {
  ...
  return {
    ...
    getDepositTxNum: prepareDepositChecking(instance),
...
function prepareDepositChecking(instance) {
  return async function ({ token, amount, txhash, owner, blocknumber }) {
    ...
    return gaslessCall(instance, 'deposit_state', owner, [hash]);
  }
}
    ...
    return fetchEvent(instance, 'NewBlockPack')
      .then(async (events) => {
        ...
        return transactions
          .map(tx => {
            return {
              BlockItems: decodePublishedBlocks(tx.input).BlockItems,
              relayer: tx.from,
              blocknumber: tx.blockNumber
            }
          });
