  ...
  return function () {
    return fetchEvent(instance, 'NewBlockPack')
      .then(async (events) => {
        ...
        return transactions.map(tx => {
          const publishedBlocksData = decodePublishedBlocks(tx.input);
          console.log(publishedBlocksData)
          return {
            BlockItems: publishedBlocksData.BlockItem,
            new_root: publishedBlocksData.new_root,
            deposit_blocknumber: publishedBlocksData.deposit_blocknumber,
            relayer: tx.from,
            blocknumber: tx.blockNumber
          }
        })
...
function decodePublishedBlocks(hex) {
  ...
  const Proof = ['uint256[8]'];
  const Message = [['uint256[4]'], ['uint256[4]']];
  const TxExternalFields = ['address', Message];
  const Tx = ['uint256', 'uint256[2]', 'uint256[2]', 'address', 'uint256', TxExternalFields, Proof];
  const BlockItem = [Tx, 'uint256', 'uint256'];
  const decodedParameters = decodeParameters([[BlockItem], 'uint', 'uint'], hex.substring(139));
  return {
    BlockItem: decodedParameters['0'],
    new_root: decodedParameters['1'],
    deposit_blocknumber: decodedParameters['2']
  }
}
