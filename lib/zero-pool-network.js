...
const { transfer_compute, utxo, nullifier, in_utxo_inputs } = require('../circom/src/inputs');
...
class ZeroPoolNetwork {
  ...
  async prepareWithdraw(token, amount) {
    const utxo = await this.calculateUtxo(BigInt(token), BigInt(this.zpKeyPair.publicKey),  BigInt(amount));
    if (utxo instanceof Error) {
      return utxo;
    }
    const preparedData = await this.prepareBlockItem(
      token,
      -utxo.utxo_in.reduce((a, b) => {
        a += b.amount;
        return a;
      }, 0n),
      utxo.utxo_in,
  ...
  async myUtxos() {
    ...
    return myUtxo.sort((a,b) => b.amount - a.amount);
  }
