...
class ZeroPoolNetwork {
  ...
  async deposit(tokenAddress, depositAmount) {
    ...
    const encoded_tx = encodeTx(preparedData.Tx);
  ...
  async withdrawal(token, utxo_in = [], utxo_out = []) {
    const preparedData = await this.prepareTransaction(
      BigInt(token),
      BigInt(BN254_ORDER - MAX_AMOUNT + 1n),
      utxo_in,
      utxo_out
    );
    preparedData.Tx.token = token;
  ...
  async myUtxos() {
    const { utxos, hashes } = await this.getUtxosFromContract();
    if (utxos.length === 0) {
      return [];
    }
